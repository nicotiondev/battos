...
Write-Host "BattOS Codex host_session smoke"
Write-Host "API: $ApiUrl"
Write-Host "Docker image: $DockerImage"
Write-Host "Codex credentials: $CodexCredentialsDir"
...
Write-Step "Checking Codex host session"
if (-not (Test-Path -LiteralPath $CodexCredentialsDir)) {
    throw "Codex credentials directory not found. Run 'codex login' on this host or set BATTOS_EXECUTION_CODEX_CREDENTIALS_DIR. Path: $CodexCredentialsDir"
}
...
if (-not $SkipImageCheck) {
    ...
    docker run --rm --network none $DockerImage bash -lc "codex --version" | Write-Host
}
...
try {
    Invoke-BattOSPost -Path "/agents" -Body @{
        slug = "codex-host-session-smoke-agent"
        name = "Codex Host Session Smoke Agent"
        role = "host session smoke test"
        runtime_id = "codex-host-session"
        risk_level = "high"
        status = "active"
    } | Out-Null
} catch {
...
try {
    ...
    $project = Invoke-BattOSPost -Path "/projects" -Body @{
        slug = "smoke-codex-host-session-$stamp"
        name = "Smoke Codex Host Session"
        status = "active"
    }
    $task = Invoke-BattOSPost -Path "/tasks" -Body @{
        project_id = $project.id
        title = "Run Codex host_session smoke"
        status = "ready"
    }
    $marker = "battos-codex-host-session-ok"
    $prompt = @"
This is a BattOS Codex host_session smoke test.
...
"@
    $run = Invoke-BattOSPost -Path "/runs" -Body @{
        project_id = $project.id
        task_id = $task.id
        agent_id = "codex-host-session-smoke-agent"
        runtime_adapter_id = "codex-host-session"
        prompt = $prompt
    ...
    $networkApproval = Invoke-BattOSPost -Path "/runs/$runId/approvals" -Body @{
        kind = "network"
        decision = "approved"
        reason = "codex host_session requires provider network access"
    }
    ...
    $executeApproval = Invoke-BattOSPost -Path "/runs/$runId/approvals" -Body @{
        kind = "execute"
        decision = "approved"
        reason = "codex host_session smoke"
    }
    ...
    $env:BATTOS_EXECUTION_CODEX_CREDENTIALS_DIR = (Resolve-Path -LiteralPath $CodexCredentialsDir).Path
    ...
    Write-Host ""
    Write-Host "BattOS Codex host_session smoke passed."
    Write-Host "Run ID: $runId"
} catch {
    Write-Host ""
    Write-Host "BattOS Codex host_session smoke failed."
    if ($runId) {
